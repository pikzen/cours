...
public:
		...
		: m_array((Pair*)malloc(sizeof(Pair)*16)), m_allocatedSize(16), m_size(0), m_message(message), m_record(false)
	{
		...
		{
			initialized = true ;
			std::cout<<"Il va vous être demandé d'entrer un réel et une chaine de caractères."<<std::endl ;
			std::cout<<"Cela est lié à des allocations effectuées par les bibliothèques de C++ qui vous seront cachées."<<std::endl ;
			std::cout<<"Entrez un réel : " ;
			{
				float f ;
				std::cin>>f ;
			}

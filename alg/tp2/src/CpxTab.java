...
public class CpxTab {
		...
		// si this[k] = x+ i*i alors  out[k] = x - i*y 
		public CpxTab conjuge() {
			//on clone this
		...
		public static CpxTab combine(CpxTab c1, CpxTab c2) {
			CpxTab tab = new CpxTab(c1.taille() * 2);
			...
			for (int k = 0; k < c1.taille(); k++)
			{
				double real = 0;
				double imgn = 0;
				double cosval = Math.cos((2*Math.PI*k)/(c1.taille()*2));
				double sinval = Math.sin((2*Math.PI*k)/(c1.taille()*2));

				real = c1.get_p_reel(k) + c2.get_p_reel(k) * cosval - c2.get_p_imag(k) * sinval;
				imgn = c1.get_p_imag(k) + c2.get_p_imag(k) * cosval + c2.get_p_reel(k) * sinval;

				tab.set_p_reel(k, real);
				tab.set_p_imag(k, imgn);
				delta = k;
			}
			delta++;
			...
			for (int k = 0; k < c1.taille(); k++)
			{
				...
				tab.set_p_reel(delta + k, real);
				tab.set_p_imag(delta + k, imgn);
			}
